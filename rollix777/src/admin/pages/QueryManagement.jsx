...
import { Search, Eye, Loader2, AlertCircle, ChevronLeft, ChevronRight, X, Send } from 'lucide-react';
import { getQueries, queryComment, queryStatus } from '../../lib/services/queryService';
...
function QueryManagement() {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      await queryComment(selectedQuery.id, { comment: comment.trim() });
  ...
  const renderMobileCard = (query) => (
    <div className="bg-[#1e1e3f] rounded-lg overflow-hidden border border-[#2f2f5a]">
      <div className="flex justify-between items-center bg-[#252547] p-2">
        ...
        <span className={`px-3 py-1 rounded-full text-sm font-medium ${statusColors[query.status]}`}>
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-white font-medium capitalize">{query.query_type}</p>
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <p className="text-white">{new Date(query.created_at).toLocaleDateString()}</p>
        </div>
  ...
  return (
    <div className="space-y-6 lg:p-6">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <h1 className="text-2xl md:text-3xl font-bold text-white">Query Management</h1>
        <button
      ...
      <div className="w-full lg:overflow-x-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 text-gray-400">
            ...
            <p className="text-sm">There are no queries matching your filters</p>
          </div>
        ) : (
          <>
            {/* Mobile View */}
            ...
            <div className="hidden md:block">
              <table className="w-full text-sm text-left text-white">
                ...
                <tbody>
                  {queries.map((query) => (
                    <tr key={query.id} className="border-b border-[#1e1e3f] hover:bg-[#2f2f5a]">
                      <td className="px-6 py-4">{query.id}</td>
                      <td className="px-6 py-4">{query.name}</td>
                      <td className="px-6 py-4">{query.email}</td>
                      <td className="px-6 py-4 capitalize">{query.query_type}</td>
                      <td className="px-6 py-4">
                        ...
                        >
                          {Object.keys(statusColors).map((status) => (
                            <option key={status} value={status} className="bg-[#1e1e3f]">
                              {status.replace('_', ' ')}
                            </option>
                          ))}
                        </select>
                      </td>
                      <td className="px-6 py-4">{new Date(query.created_at).toLocaleDateString()}</td>
                      <td className="px-6 py-4">
                        <button
                          onClick={() => handleViewDetails(query)}
        ...
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-4 overflow-y-auto">
          ...
          <div className="relative w-full max-w-3xl bg-gradient-to-b from-[#252547] to-[#1A1A2E] rounded-2xl overflow-hidden animate-fadeIn my-4">
            ...
            <div className="flex justify-between items-center p-4 border-b border-purple-500/10">
              <h2 className="text-lg sm:text-xl font-bold text-white">Query Details - {selectedQuery.id}</h2>
              <button
            ...
            <div className="p-6 space-y-4">
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Name:</span>
                <span className="text-sm text-white">{selectedQuery.name}</span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Email:</span>
                <span className="text-sm text-white">{selectedQuery.email}</span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Phone:</span>
                <span className="text-sm text-white">{selectedQuery.phone}</span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Telegram:</span>
                <span className="text-sm text-white">{selectedQuery.telegram_id}</span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Type:</span>
                <span className="text-sm text-white capitalize">{selectedQuery.query_type}</span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Message:</span>
                <span className="text-sm text-white">{selectedQuery.message}</span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Status:</span>
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[selectedQuery.status]}`}>
                  {selectedQuery.status.replace('_', ' ')}
                </span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Created At:</span>
                <span className="text-sm text-white">{new Date(selectedQuery.created_at).toLocaleString()}</span>
              </div>
              <div className="flex">
                <span className="w-32 text-sm font-medium text-gray-400">Comments:</span>
                <span className="text-sm text-white">{selectedQuery.comment_count}</span>
              </div>
