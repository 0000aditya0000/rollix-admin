...
const KYCRequest = () => {
  ...
  const [allKYCRequests, setAllKYCRequests] = useState<KYCRequest[]>([]);
  ...
  const fetchKYCRequests = async (page: number) => {
    try {
      ...
      if (searchTerm.trim()) {
        url.searchParams.append("phone", searchTerm.trim());
      }

      const response = await axios.get<PaginatedResponse>(url.toString());

      if (response.data.success) {
        setAllKYCRequests(response.data.data || []);
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      if (searchTerm.trim() === "") {
        setKYCRequests(allKYCRequests); // Show all if empty
      } else {
        const filtered = allKYCRequests.filter((request) =>
          request.phone.includes(searchTerm.trim())
        );
        setKYCRequests(filtered);
      }
    }, 300);

    return () => clearTimeout(delayDebounce);
  }, [searchTerm, allKYCRequests]);
