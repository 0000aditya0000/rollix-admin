...
import { Users, DollarSign, Eye, Check, X, User, IndianRupeeIcon, Loader2 } from 'lucide-react';
          ...
          {activeTab === 'kyc' ? (
            <div className="overflow-x-auto">
              {loadingKYC ? (
                ...
                <div className="text-center text-gray-400 py-8">No pending KYC requests</div>
              ) : (
                <table className="w-full">
                  ...
                  <tbody>
                    {kycRequests.map((request) => (
                      <tr key={request.user_id} className="border-t border-purple-500/10 text-white">
                        <td className="py-4">#{request.user_id}</td>
                        <td className="py-4">{request.name || request.username}</td>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {withdrawalRequests.map((request) => (
                      <tr key={request.withdrawalId} className="border-t border-purple-500/10 text-white">
                        <td className="py-4">#{request.withdrawalId}</td>
                        <td className="py-4">{request.user.name || request.user.username}</td>
                        <td className="py-4">
                          <span className="font-medium">
                            {Number(request.amount).toLocaleString()} {request.cryptoname.toUpperCase()}
                          </span>
                        </td>
                        <td className="py-4">
                          <span className="font-medium">
                            {'walletAddress' in request.withdrawalDetails ? 'CRYPTO' : 'BANK TRANSFER'}
                          </span>
                        </td>
                        <td className="py-4">{formatDate(request.requestDate)}</td>
                        <td className="py-4">
                          <span className={`font-medium ${getStatusColor(request.withdrawalStatus.code)}`}>
